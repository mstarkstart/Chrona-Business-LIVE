...
const ACTIONS: Record<string, ActionDef> = {
  "task.create":     { label: "Create / approve task",  icon: ClipboardCheck, href: "/tasks?new=1",         color: "#4f46e5" },
  "ai.chat":         { label: "Talk to AI assistant",   icon: Sparkles,       comingSoon: true,             color: "#a78bfa" },
  "report.generate": { label: "Generate report",        icon: FileBarChart,   comingSoon: true,             color: "#6366f1" },
  "calendar.new":    { label: "New calendar event",     icon: CalendarPlus,   href: "/calendar",            color: "#10b981" },
  ...
  return (
    <>
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col items-end gap-3">
        ...
        {open && (
          <div className="flex flex-col items-end gap-2 mb-2">
            {items.map((key, i) => {
              ...
              const content = (
                <div
                  className="flex items-center gap-3 rounded-2xl border border-border bg-white px-4 py-3 shadow-lg shadow-gray-200 text-sm font-medium hover:bg-accent transition-all animate-fade-up cursor-pointer"
                  style={{ animationDelay: `${i * 40}ms` }}
                  onClick={() => pick(key)}
                >
                  <div className="h-7 w-7 rounded-xl flex items-center justify-center" style={{ background: `${def.color}18` }}>
                    <Icon className="h-3.5 w-3.5" style={{ color: def.color }} />
                  </div>
                  {def.label}
                  {def.comingSoon && (
                    <span className="ml-1 text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">soon</span>
                  )}
                </div>
              );
        ...
        <button
          onClick={() => setOpen((o) => !o)}
          className={`h-12 w-12 rounded-2xl flex items-center justify-center shadow-xl transition-all active:scale-95 ${
            open
              ? "bg-white border border-border text-foreground"
              : "bg-primary text-white glow-primary hover:brightness-110"
          }`}
        >
