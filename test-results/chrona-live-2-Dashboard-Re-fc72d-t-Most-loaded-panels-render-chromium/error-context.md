...
> 92  |     await expect(page.locator("text=Most efficient")).toBeVisible();
  ...
  187 |     await login(page, USER_A);
  188 |     await page.goto(`${BASE_URL}/tasks`);
  189 |   });
  190 | 
  191 |   test("4a. Tasks page renders list view with tasks", async ({ page }) => {
  192 |     await expect(page.locator("h1")).toContainText(/Task/i);
```